...
### 4f. Facilitator control plane

The clean architecture is not `user -> LLM -> text`. It is:

`human intent -> facilitator policy layer -> role selection / trigger matrix -> structured proposal -> execute or stage -> board/bead mutation -> human accept/override/undo`

Treat the robot as a bounded managerial actor layered over the board, bead flow, and WebMCP verbs.

- **Purpose.** The facilitator exists to research, challenge, synthesize, preserve memory, and drive momentum. Those are managerial behaviors, not assistant flavor text.
- **Control-plane stack.**
  - `Workspace layer` — the board state is the source of truth: ideas, groups, docs, suggestions, critiques, connections, turn logs, and change sets
  - `Workflow layer` — the bead system is the structured thinking path; `advance_phase` stays the only hard transition
  - `Tool layer` — every meaningful robot move maps to an explicit WebMCP verb
  - `Role layer` — the LLM runs only as `Scout`, `Synthesizer`, `Challenger`, `Historian`, or `Facilitator`
  - `Policy layer` — autonomy ceiling, effective mode, cooldowns, trigger matrix, per-role permissions, per-bead guardrails, trust signals
  - `Execution layer` — proposals dispatch through the event/tool handshake or get staged
  - `Oversight layer` — humans can dismiss, admit, elaborate, override, manually advance, or undo robot actions
- **Facilitator Policy Engine.** Add an explicit layer that answers four questions for every candidate action:
  - should the robot act?
  - which role should act?
  - should the output execute or stage?
  - how strong should the intervention be?
- **Proposal-first default.** The default managerial stance should be proposal-first, not mutation-first. Directional or potentially interruptive behavior should usually stage into Shadow/Robot's Notes before it mutates the board.
- **Research enriches the board, not hidden AI state.** When the robot reads docs, active-tab context, or outside knowledge, the result should become attached docs, candidate ideas, review flags, rationale, or connection proposals on the board rather than private chat context.
- **Learning means policy adaptation.** The system should not claim the model “got smarter.” It should claim that staging, permissions, and trigger thresholds adapt based on recent accept/reject outcomes.
- **Direction comes from evidence.** The robot should derive its direction from board structure, discarded memory, docs, tensions, critiques, and bead progression rather than from obedience to the loudest recent prompt.

**Trigger Matrix**

The orchestrator should use a lightweight rules engine that watches facilitator context and decides which role may emit a proposal next.

Concrete trigger classes:

- `Scout` — unresolved ambiguity with no docs, active-tab context that changes the option space, or external systems mentioned without supporting evidence
- `Synthesizer` — 3+ overlapping ideas, contradiction across clusters, or fragmentation after rapid idea creation
- `Challenger` — dominant idea with no critique, too-fast convergence, or phase 4+ with open risk and no stress handling
- `Historian` — stalled board plus a discarded idea that matches a new doubt, critique, or revived theme
- `Facilitator` — clear next bead, cycling without progress, or consensus reached with no captured next step
...
type Trigger = {
  id: string;
  when: (ctx: FacilitatorContext) => boolean;
  role: 'scout' | 'synthesizer' | 'challenger' | 'historian' | 'facilitator';
  priority: number;
  mode: 'execute_if_allowed' | 'stage_only';
  cooldownMs?: number;
};
```

This is how the product gets “insinuated, not forced.”

### 4g. Dynamic autonomy engine
...
### 4h. Technical execution loop — Node + React
