...
export function BeatReviewPanelCard({
  ...
  return (
    <article className="bo-card-surface rounded-2xl p-4 shadow-sm">
      ...
      <div className="mt-3 space-y-3">
        ...
        {item.status !== 'pending' && (
          <div className={`rounded-2xl border px-3 py-2 text-xs leading-snug ${
            item.status === 'kept'
              ? 'border-emerald-200 bg-emerald-50 text-emerald-800'
              : 'border-slate-300 bg-slate-100 text-slate-600'
          }`}>
            {item.status === 'kept'
              ? 'Kept. This candidate has been applied or committed for the current board state.'
              : 'Scratched. This candidate is preserved for review history but will not change the board.'}
          </div>
        )}
      ...
      <div className="mt-4 flex items-center justify-end gap-2">
        ...
        >
          {item.status === 'scratched' ? 'Scratched' : 'Scratch'}
        </button>
        <button
          type="button"
